...
-- Subversion Repository: http://arcadmin.googlecode.com/svn/
--
-------------------------------------------------------------------------------------------------------------
...
--      ma_infomaxonlinetext:SetText(Locale["info_maxonline"]..maxusers)
    ...
    for uptime in string.gmatch(text, Strings["ma_GmatchUptime"]) do
      uptime = string.gsub(uptime, "days", "d")
      uptime = string.gsub(uptime, "hours", "h")
      uptime = string.gsub(uptime, "minutes", "m")
      uptime = string.gsub(uptime, "seconds", "s")
      ma_infouptimetext:SetText(Locale["info_uptime"]..uptime)
    end
    for latency in string.gmatch(text, Strings["ma_GmatchLatency"]) do
      ma_infomaxonlinetext:SetText(Locale["info_maxonline"]..latency)
    end
...
function MangAdmin:ToggleFlyMode(value)
  --if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    ...
    MangAdmin:ChatMsg(".cheat fly "..value)
...
function MangAdmin:ToggleHoverMode(value)
  ...
  MangAdmin:ChatMsg(".cheat god  "..status)
  MangAdmin:LogAction("Turned Hover-mode "..status..".")
end

function MangAdmin:ToggleWhisper(value)
  MangAdmin:ChatMsg(".invincible ")
  MangAdmin:LogAction("Toggled Invincibility.")
end

function MangAdmin:ToggleVisible(value)
  MangAdmin:ChatMsg(".invisible "..value)
  if value == "on" then
...
function MangAdmin:AddItem(value, state)
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    ...
    if state == "RightButton" then
      ...
      if amount == "" then
        self:ChatMsg(".character additem "..value)
        self:LogAction("Added item with id "..value.." to "..player..".")
      else
        self:ChatMsg(".character additem "..value.." "..amount)
...
function MangAdmin:KickPlayer()
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg(".kick player "..player)
...
function MangAdmin:Distance()
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg(".debug dist")
    self:LogAction("Got distance to player "..player..".")
end

function MangAdmin:SetJail_A()
    self:ChatMsg(".recall del ma_AllianceJail")
    i=1
    while i<30 do
        i=i+1
        self:ChatMsg("Creating Jail "..i)
    end
    self:ChatMsg(".recall add ma_AllianceJail")
    self:LogAction("Set location of Alliance Jail")
end

function MangAdmin:SetJail_H()
    self:ChatMsg(".recall del ma_HordeJail")
    i=1
    while i<30 do
        i=i+1
        self:ChatMsg("Creating Jail "..i)
    end
    self:ChatMsg(".recall add ma_HordeJail")
    self:LogAction("Set location of Horde Jail")
end

function MangAdmin:JailA()
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg("Selected "..player)
    self:ChatMsg(".recall portplayer "..player.." ma_AllianceJail")
    self:LogAction("Jailed player "..player..".")
    self:ChatMsg(".wannounce "..player.." has been found guilty and jailed.")
end

function MangAdmin:JailH()
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg("Selected "..player)
    self:ChatMsg(".recall portplayer "..player.." ma_HordeJail")
    self:LogAction("Jailed player "..player..".")
    self:ChatMsg(".wannounce "..player.." has been found guilty and jailed.")
end
...
function MangAdmin:UnJail()
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg("Selected "..player)
    self:ChatMsg(".recall portplayer "..player.." BootyBay")
    self:LogAction("Un-Jailed player "..player..".")
    self:ChatMsg(".wannounce "..player.." has been pardoned and released from jail in Booty Bay.")
end
...
function MangAdmin:Mount()
  local cname = ma_gridnavieditbox:GetText()
  self:ChatMsg(".mount "..cname)
  self:LogAction("Mounted: "..cname..".")
end
...
function MangAdmin:ToggleTickets(value)
  MangAdmin:ChatMsg(".gmticket toggle "..value)
...
function MangAdmin:InitButtons()
  ...
  self:PrepareScript(ma_displaylevelbutton   , Locale["tt_DisplayAccountLvl"]  , function() MangAdmin:ChatMsg(".playerinfo") end)
...
--  self:PrepareScript(ma_invisibleoffbutton   , Locale["tt_InvisOffButton"]     , function() MangAdmin:ToggleVisible("on") end)
  ...
  self:PrepareScript(ma_ticketonbutton       , Locale["tt_TicketOn"]           , function() MangAdmin:ToggleTickets("") end)
--  self:PrepareScript(ma_ticketoffbutton      , Locale["tt_TicketOff"]          , function() MangAdmin:ToggleTickets("") end)
  self:PrepareScript(ma_mapsonbutton         , Locale["tt_ShowMapsButton"]     , function() MangAdmin:ToggleMaps("on") end)
  self:PrepareScript(ma_mapsoffbutton        , Locale["tt_HideMapsButton"]     , function() MangAdmin:ToggleMaps("off") end)
  self:PrepareScript(ma_bankbutton           , Locale["tt_BankButton"]         , function() MangAdmin:Mount() end)
  ...
  self:PrepareScript(ma_showmapsbutton       , Locale["tt_ShowMapsButton"]     , function() MangAdmin:ToggleMaps("on") end)
  self:PrepareScript(ma_hidemapsbutton       , Locale["tt_HideMapsButton"]     , function() MangAdmin:ToggleMaps("off") end)
...
function MangAdmin:InitSliders()
  ...
  ma_speedslider:SetMinMaxValues(1, 25)
