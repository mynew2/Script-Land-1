...
MAJOR_VERSION = "ArcAdmin-3.3.5a"
MINOR_VERSION = "$Revision: 2 $"
ROOT_PATH     = "Interface\\AddOns\\ArcAdmin\\"
local cont = ""
if not AceLibrary then error(MAJOR_VERSION .. " requires AceLibrary") end
if not AceLibrary:IsNewVersion(MAJOR_VERSION, MINOR_VERSION) then return end

ArcAdmin    = AceLibrary("AceAddon-2.0"):new("AceConsole-2.0", "AceDB-2.0", "AceHook-2.1", "FuBarPlugin-2.0", "AceDebug-2.0", "AceEvent-2.0")
Locale       = AceLibrary("AceLocale-2.2"):new("ArcAdmin")
Strings      = AceLibrary("AceLocale-2.2"):new("TEST")
FrameLib     = AceLibrary("FrameLib-1.0")
Graph        = AceLibrary("Graph-1.0")
local Tablet = AceLibrary("Tablet-2.0")

ArcAdmin:RegisterDB("ArcAdminDb", "ArcAdminDbPerChar")
ArcAdmin:RegisterDefaults("char", 
  {
    functionQueue = {},
    requests = {
      tpinfo = false,
...
ArcAdmin:RegisterDefaults("account", 
  {
    language = nil,
...
ArcAdmin.consoleOpts = {
  ...
  args = {
    toggle = {
      ...
      type = 'execute',
      func = function() ArcAdmin:OnClick() end
    },
    transparency = {
      name = "transparency",
      desc = Locale["cmd_transparency"],
      type = 'execute',
      func = function() ArcAdmin:ToggleTransparency() end
    },
    tooltips = {
      name = "tooltips",
      desc = Locale["cmd_tooltip"],
      type = 'execute',
      func = function() ArcAdmin:ToggleTooltips() end
    },
    minimenu = {
      name = "tooltips",
      desc = "Toogle the toolbar/minimenu",
      type = 'execute',
      func = function() ArcAdmin:ToggleMinimenu() end
    }
  }
}

function ArcAdmin:OnInitialize()
  -- initializing ArcAdmin
  self:SetLanguage()
  self:CreateFrames()
  self:RegisterChatCommand(Locale["slashcmds"], self.consoleOpts) -- this registers the chat commands
  self:InitButtons()  -- this prepares the actions and tooltips of nearly all ArcAdmin buttons  
  InitControls()
  self:SearchReset()
  ArcAdmin.db.account.buffer.who = {}
  -- FuBar plugin config
  ArcAdmin.hasNoColor = true
  ArcAdmin.hasNoText = false
  ArcAdmin.clickableTooltip = true
  ArcAdmin.hasIcon = true
  ArcAdmin.hideWithoutStandby = true
  ArcAdmin:SetIcon(ROOT_PATH.."Textures\\icon.tga")
  -- make ArcAdmin frames closable with escape key
  ...
  -- They will be redirected to ArcAdmin:AddMessage(...)
  ...
  ArcLinkifier_SetItemRef_Original = SetItemRef
  SetItemRef = ArcLinkifier_SetItemRef
...
--events
function ArcAdmin:PLAYER_DEAD()
  ma_mm_revivebutton:Show()
end

function ArcAdmin:PLAYER_ALIVE()
  ma_mm_revivebutton:Hide()
end

function ArcAdmin:ZONE_CHANGED()
  --[[if hastranslationlocale then
    if not ArcAdmin.db.char.selectedZone or ArcAdmin.db.char.selectedZone ~= translate(GetZoneText()) then
      if translationfor(GetZoneText()) then
        ArcAdmin.db.char.selectedZone = translate(GetZoneText())
        InlineScrollUpdate()
      end
    end
  end]]
...
function ArcAdmin:UNIT_MODEL_CHANGED()
  ModelChanged()
end

function ArcAdmin:PLAYER_TARGET_CHANGED()
  ModelChanged()
  NpcModelChanged()
...
function ArcAdmin:OnDisable()
  -- called when the addon is disabled
  self:SearchReset()
end

function ArcAdmin:OnClick()
  -- this toggles the ArcAdmin frame when clicking on the mini icon
  if IsShiftKeyDown() then
    ReloadUI()
  elseif IsAltKeyDown() then
    self.db.char.newTicketQueue = 0
    ArcAdmin:UpdateTooltip()
...
function ArcAdmin:OnTooltipUpdate()
  ...
  if ticketCount == 0 then
    ...
    cat:AddLine(
      ...
      "func", function() ArcAdmin:ShowTicketTab() end)
    local counter = 0
    local name
    for i, name in pairs(tickets) do
      ...
      if counter == ticketCount then
        cat:AddLine(
          ...
          "func", function(name) ArcAdmin:TelePlayer("gochar", name) end,
          "arg1", name
        )
        cat:AddLine(
          "text", string.format(Locale["ma_TicketsGetLast"], name),
          "func", function(name) ArcAdmin:TelePlayer("getchar", name) end,
          "arg1", name
        )
      end
    end
    ArcAdmin:SetIcon(ROOT_PATH.."Textures\\icon2.tga")
  end
  Tablet:SetHint(Locale["ma_IconHint"])
end

function ArcAdmin:ToggleTabButton(group)
...
function ArcAdmin:ToggleContentGroup(group)
  --ArcAdmin:LogAction("Toggled navigation point '"..group.."'.")
  self:HideAllGroups()
  FrameLib:HandleGroup(group, function(frame) frame:Show() end)
end

function ArcAdmin:InstantGroupToggle(group)
  if group == "ticket" then
    self.db.char.requests.ticket = false
  end
  if group== "who" then
    ArcAdmin:ChatMsg(".server info")
    ResetWho()
  end
  FrameLib:HandleGroup("bg", function(frame) frame:Show() end)
  ArcAdmin:ToggleTabButton(group)
  ArcAdmin:ToggleContentGroup(group)
end

function ArcAdmin:TogglePopup(value, param)
  -- this toggles the ArcAdmin Search Popup frame, toggling deactivated, popup will be overwritten
  ...
  if value == "search" then
    ...
    ma_resetsearchbutton:SetScript("OnClick", function() ArcAdmin:SearchReset() end)
    ma_resetsearchbutton:SetText(Locale["ma_ResetButton"])
    ma_resetsearchbutton:Enable()
    ma_ptabbutton_1:SetScript("OnClick", function() ArcAdmin:TogglePopup("search", {type = param.type}) end)
    ma_ptabbutton_2:SetScript("OnClick", function() ArcAdmin:TogglePopup("favorites", {type = param.type}) end)
    ...
    elseif param.type == "ticket" then
      ...
      ma_resetsearchbutton:SetScript("OnClick", function() ArcAdmin.db.account.tickets.loading = true; self:LoadTickets(ArcAdmin.db.account.tickets.count) end)]]--
    end
  ...
  elseif value == "mail" then
    ...
    ma_searcheditbox:SetScript("OnTextChanged", function() ArcAdmin:UpdateMailBytesLeft() end)
    ma_var1editbox:SetScript("OnTextChanged", function() ArcAdmin:UpdateMailBytesLeft() end)
    ma_modfavsbutton:Hide()
    ma_selectallbutton:Hide()
    ma_deselectallbutton:Hide()
...
function ArcAdmin:HideAllGroups()
...
function ArcAdmin:TicketHackTimer()
...
function ArcAdmin:AddMessage(frame, text, r, g, b, id)
  -- frame is the object that was hooked (one of the ChatFrames)  
  local catchedSth = false
  local output = ArcAdmin.db.account.style.showchat
...
--[[            for w in string.gmatch(WorkString,'%s.%d*%p%d%d') do
        ...
        --Send the port
        SendChatMessage('.worldport '..' '..nX..' '..nY..' '..(cZ+incZ))
        --console reloadui
        incX = 0
        incY = 0
        incZ = 0
        ...
        if isChecked == 1 then  --AddonMove
            ...
            SendChatMessage('.gobject spawn '..ObjectN)
        elseif isChecked2 == 1 then --MoveonMove
            SendChatMessage('.gobject del '..ma_Obj_guidbutton:GetText())
            ObjectN = ma_Obj_idbutton:GetText()
            SendChatMessage('.gobject spawn '..ObjectN)
...
--***************************************************************    
    ...
    if ArcAdmin:ID_Setting_Start_Read() then    
        ...
        if b1 then
            ...
            if b1 then
                ...
                ArcAdmin:ID_Setting_Write(1,pattern)
    ...
    if ArcAdmin:OID_Setting_Start_Read() then    
        local b1,e1,pattern = string.find(text, "GUID: (%d+) ")
        --local b1,e1,pattern = string.find(text, "GUID:")
        if b1 then
            b1,e1,pattern = string.find(text, "([0-9]+)")
            if b1 then
                ArcAdmin:OID_Setting_Start_Write(0)
                
                ArcAdmin:OID_Setting_Write(0,pattern)
    ...
    --      		ArcAdmin:OID_Setting_Write(1,pattern)
    ...
    if ArcAdmin:Way_Point_Add_Start_Read() then    
        b1,e1,pattern = string.find(text, "Waypoint (%d+)")
        if b1 then
            ArcAdmin:Way_Point_Add_Start_Write(0)
            
            local wnpc =	ma_NPC_guidbutton:GetText()
            self:ChatMsg(".waypoint show "..wnpc)
    ...
    if self.db.char.requests.toggle then
      if self.db.char.requests.item then
        ...
        for id, name in string.gmatch(text, Strings["ma_GmatchItem"]) do
            ...
            PopupScrollUpdate()
            catchedSth = true
            output = ArcAdmin.db.account.style.showchat  
        end
...
--        output = ArcAdmin.db.account.style.showchat
    ...
    for id, char, create, update in string.gmatch(text, Strings["ma_GmatchTickets"]) do
        table.insert(ArcAdmin.db.account.buffer.tickets, {tNumber = id, tChar = char, tLCreate = create, tLUpdate = update, tMsg = ""})
        local ticketCount = 0
        table.foreachi(ArcAdmin.db.account.buffer.tickets, function() ticketCount = ticketCount + 1 end)
        ticketCount = 0
        catchedSth = true
        output = ArcAdmin.db.account.style.showchat
    ...
    for msg in string.gmatch(text, "Ticket Message.-:.-(.*)") do
        ArcAdmin.db.account.buffer.ticketread=true
        ArcAdmin.db.account.buffer.ticketsfull = {}
        table.remove(ArcAdmin.db.account.buffer.ticketsfull, 1)
        --table.insert(ArcAdmin.db.account.buffer.ticketsfull, {tMsg = msg})
        table.insert(ArcAdmin.db.account.buffer.ticketsfull, {tMsg = " "})
        ma_ticketdetail:SetText("|cffffffff"..msg)
        catchedSth = true
        output = ArcAdmin.db.account.style.showchat
    end       

    if ArcAdmin.db.account.buffer.ticketread==true then
        for msg in string.gmatch(text, "(.*)]|r") do
            local object = ArcAdmin.db.account.buffer.ticketsfull[1]
            local t_msg = ""
            t_msg = object["tMsg"]
            t_msg = t_msg.." ".."|cffffffff"..msg
            table.remove(ArcAdmin.db.account.buffer.ticketsfull, 1)
            table.insert(ArcAdmin.db.account.buffer.ticketsfull, {tMsg = t_msg})
            ArcAdmin.db.account.buffer.ticketread=false
    ...
    for acc, char, ip, map, zone, exp, gmlevel in string.gmatch(text, Strings["ma_GmatchWho"]) do
        ...
        if acc == "Account" then
            ...
            table.insert(ArcAdmin.db.account.buffer.who, {tAcc = acc, tChar = char, tIP = ip, tMap = map, tZone = zone, tExp = exp, tGMLevel = gmlevel})
        end
            catchedSth = true
            output = ArcAdmin.db.account.style.showchat
            WhoUpdate()
 ...
 --[[   
    ...
    if catchedSth then
      ...
      elseif output == true then
        text = ArcLinkifier_Decompose(text)
...
--[[ function ArcAdmin:GetValueCallHandler(guid, field, value)
  ...
  elseif guid == realGuid then
    ...
    ArcAdmin:LogAction("DEBUG: Getvalues are: GUID = "..guid.."; field = "..field.."; value = "..value..";")
    return true
  end
end ]]

function ArcAdmin:LogAction(msg)
  ma_logframe:AddMessage("|cFF00FF00["..date("%H:%M:%S").."]|r "..msg, 1.0, 1.0, 0.0)
end


function ArcAdmin:ChatMsg(msg, msgt, recipient)
...
function ArcAdmin:Selection(selection)
...
function ArcAdmin:AndBit(value, test) 
  return mod(value, test*2) >= test 
end


--=================================--
--ArcAdmin Commands functions--
--=================================--
function ArcAdmin:SetLanguage()
...
function ArcAdmin:SetSkill(value, skill, maxskill)
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    ...
    if type(value) == "string" then
      self:ChatMsg(".char advanceskill "..value.." "..skill.." "..maxskill)
      self:LogAction("Set skill "..maxskill" of "..player.." to "..skill.." with a maximum of "..maxskill..".")
...
function ArcAdmin:Quest(value, state)
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    local player = UnitName("target") or UnitName("player")
    local class = UnitClass("target") or UnitClass("player")
    local command = ".quest start"
    local logcmd = "Started"
    local logcmd2 = "to"
...
function ArcAdmin:Creature(value, state)
    local command = ".npc spawn"
...
function ArcAdmin:AddItem(value, state)
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    local player = UnitName("target") or UnitName("player")
    local amount = ma_var1editbox:GetText()
    if state == "RightButton" then
      if amount == "" then
        self:ChatMsg(".char additem "..value.." -1")
    ...
    --      self:LogAction("Listed item with id "..value..".")
        ...
        self:ChatMsg(".char additem "..value.." "..amount)
      ...
      if amount == "" then
        self:ChatMsg(".char additem "..value)
...
function ArcAdmin:AddItemSet(value)
  if self:Selection("player") or self:Selection("self") or self:Selection("none") then
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg(".char additemset "..value)
...
local Arc_Waypoint_start = 0
local Arc_ID_start = 0
local Arc_ID_guid = ""
local Arc_ID_entryid = ""
local Arc_OID_start = 0
local Arc_OID_guid = ""
local Arc_OID_entryid = ""

function ArcAdmin:Way_Point_Add_Start_Write(num)

    Arc_Waypoint_start = num

end

function ArcAdmin:Way_Point_Add_Start_Read()

    return Arc_Waypoint_start

end

function ArcAdmin:ID_Setting_Start_Write(num)
    
    Arc_ID_start = num

end    

function ArcAdmin:ID_Setting_Start_Read()
    
    return Arc_ID_start

end    

function ArcAdmin:ID_Setting_Write(num,val)
...
function ArcAdmin:ID_Setting_Read(num)
...
function ArcAdmin:OID_Setting_Start_Write(num)
    
    Arc_OID_start = num

end    

function ArcAdmin:OID_Setting_Start_Read()
    
    return Arc_OID_start

end    

function ArcAdmin:OID_Setting_Write(num,val)
...
function ArcAdmin:OID_Setting_Read(num)
...
function ArcAdmin:NPCAdd_Way_o()                            
    local player = UnitName("target") or UnitName("player")
    local npc =	ma_NPC_guidbutton:GetText()
    self:ChatMsg(".waypoint add "..npc)
    self:ChatMsg(".waypoint show "..npc)
    self:LogAction("WayPoint Add for player "..player..".")
end

function ArcAdmin:WayModify()
    local player = UnitName("target") or UnitName("player")
    self:ChatMsg(".npc info")
    self:LogAction("Got NPC info for player "..player..".")
end

function ArcAdmin:NPC_GUID_Get_org()                            
    local player = UnitName("target") or UnitName("player") 
...
function ArcAdmin:CreateGuild(leader, name)
  self:ChatMsg(".guild create "..leader.." "..name)
  self:LogAction("Created guild '"..name.."' with leader "..leader..".")
end

function ArcAdmin:SendMail(recipient, subject, body)
...
function ArcAdmin:UpdateMailBytesLeft()
...
function ArcAdmin:Favorites(value, searchtype)
  if value == "add" then
...
function ArcAdmin:SearchStart(var, value)
  self.db.char.requests.toggle = true
  if var == "item" then
...
function ArcAdmin:PrepareScript(object, text, script)
  --if object then
    if text then
...
function ArcAdmin:InitButtons()
  -- start tab buttons
  self:PrepareScript(ma_tabbutton_main       , Locale["tt_MainButton"]         , function() ArcAdmin:InstantGroupToggle("main") end)
  self:PrepareScript(ma_tabbutton_char       , Locale["tt_CharButton"]         , function() ArcAdmin:InstantGroupToggle("char") end)
  self:PrepareScript(ma_tabbutton_char2      , Locale["tt_Char2Button"]        , function() ArcAdmin:InstantGroupToggle("char2") end)
  self:PrepareScript(ma_tabbutton_npc        , Locale["tt_NpcButton"]          , function() ArcAdmin:InstantGroupToggle("npc"); end)
  self:PrepareScript(ma_tabbutton_npc2       , Locale["tt_Npc2Button"]         , function() ArcAdmin:InstantGroupToggle("npc2"); end)
  self:PrepareScript(ma_tabbutton_go         , Locale["tt_GOButton"]           , function() ArcAdmin:InstantGroupToggle("go"); end)
  self:PrepareScript(ma_tabbutton_tele       , Locale["tt_TeleButton"]         , function() ArcAdmin:InstantGroupToggle("tele"); end)
  self:PrepareScript(ma_tabbutton_misc       , Locale["tt_MiscButton"]         , function() ArcAdmin:InstantGroupToggle("misc") end)
  self:PrepareScript(ma_tabbutton_server     , Locale["tt_ServerButton"]       , function() ArcAdmin:InstantGroupToggle("server") end)
  self:PrepareScript(ma_tabbutton_log        , Locale["tt_LogButton"]          , function() ArcAdmin:InstantGroupToggle("log") end)
  self:PrepareScript(ma_tabbutton_pvp        , nil          , function() ArcAdmin:InstantGroupToggle("pvp") end)
  self:PrepareScript(ma_tabbutton_event      , nil          , function() ArcAdmin:InstantGroupToggle("event") end)
  self:PrepareScript(ma_tabbutton_rpg        , nil          , function() ArcAdmin:InstantGroupToggle("rpg") end)
  self:PrepareScript(ma_tabbutton_vendor     , nil          , function() ArcAdmin:InstantGroupToggle("vendor") end)
  self:PrepareScript(ma_tabbutton_ahbot      , nil          , function() ArcAdmin:InstantGroupToggle("ahbot") end)
  self:PrepareScript(ma_tabbutton_who        , nil          , function() ArcAdmin:InstantGroupToggle("who") end)
  --end tab buttons
  -- start mini buttons
  self:PrepareScript(ma_mm_logoframe         , nil                             , function() ArcAdmin:OnClick() end)
  self:PrepareScript(ma_mm_mainbutton        , Locale["tt_MainButton"]         , function() ArcAdmin:InstantGroupToggle("main") end)
  self:PrepareScript(ma_mm_charbutton        , Locale["tt_CharButton"]         , function() ArcAdmin:InstantGroupToggle("char") end)
  self:PrepareScript(ma_mm_char2button       , Locale["tt_Char2Button"]        , function() ArcAdmin:InstantGroupToggle("char2") end)
  self:PrepareScript(ma_mm_npcbutton         , Locale["tt_NpcButton"]          , function() ArcAdmin:InstantGroupToggle("npc") end)
  self:PrepareScript(ma_mm_npc2button        , Locale["tt_Npc2Button"]         , function() ArcAdmin:InstantGroupToggle("npc2") end)
  self:PrepareScript(ma_mm_gobutton          , Locale["tt_GOButton"]           , function() ArcAdmin:InstantGroupToggle("go") end)
  self:PrepareScript(ma_mm_telebutton        , Locale["tt_TeleButton"]         , function() ArcAdmin:InstantGroupToggle("tele") end)
  self:PrepareScript(ma_mm_ticketbutton      , Locale["tt_TicketButton"]       , function() ShowTicketTab() end)
  self:PrepareScript(ma_mm_miscbutton        , Locale["tt_MiscButton"]         , function() ArcAdmin:InstantGroupToggle("misc") end)
  self:PrepareScript(ma_mm_serverbutton      , Locale["tt_ServerButton"]       , function() ArcAdmin:InstantGroupToggle("server") end)
  self:PrepareScript(ma_mm_logbutton         , Locale["tt_LogButton"]          , function() ArcAdmin:InstantGroupToggle("log") end)
  self:PrepareScript(ma_mm_pvpbutton        , nil          , function() ArcAdmin:InstantGroupToggle("pvp") end)
  self:PrepareScript(ma_mm_eventbutton      , nil          , function() ArcAdmin:InstantGroupToggle("event") end)
  self:PrepareScript(ma_mm_rpgbutton        , nil          , function() ArcAdmin:InstantGroupToggle("rpg") end)
  self:PrepareScript(ma_mm_vendorbutton     , nil          , function() ArcAdmin:InstantGroupToggle("vendor") end)
  self:PrepareScript(ma_mm_ahbotbutton      , nil          , function() ArcAdmin:InstantGroupToggle("ahbot") end)
  self:PrepareScript(ma_mm_whobutton        , nil          , function() ArcAdmin:InstantGroupToggle("who") end)
  --end mini buttons
  self:PrepareScript(ma_languagebutton       , Locale["tt_LanguageButton"]     , function() ArcAdmin:ChangeLanguage(UIDropDownMenu_GetSelectedValue(ma_languagedropdown)) end)
  self:PrepareScript(ma_itembutton           , Locale["tt_ItemButton"]         , function() ArcAdmin:TogglePopup("search", {type = "item"}) end)
  self:PrepareScript(ma_itemsetbutton        , Locale["tt_ItemSetButton"]      , function() ArcAdmin:TogglePopup("search", {type = "itemset"}) end)
  self:PrepareScript(ma_spellbutton          , Locale["tt_SpellButton"]        , function() ArcAdmin:TogglePopup("search", {type = "spell"}) end)
  self:PrepareScript(ma_skillbutton          , Locale["tt_SkillButton"]        , function() ArcAdmin:TogglePopup("search", {type = "skill"}) end)
  self:PrepareScript(ma_questbutton          , Locale["tt_QuestButton"]        , function() ArcAdmin:TogglePopup("search", {type = "quest"}) end)
  self:PrepareScript(ma_creaturebutton       , Locale["tt_CreatureButton"]     , function() ArcAdmin:TogglePopup("search", {type = "creature"}) end)
  self:PrepareScript(ma_objectbutton         , Locale["tt_ObjectButton"]       , function() ArcAdmin:TogglePopup("search", {type = "object"}) end)
  self:PrepareScript(ma_telesearchbutton     , Locale["ma_TeleSearchButton"]   , function() ArcAdmin:TogglePopup("search", {type = "tele"}) end)
  self:PrepareScript(ma_sendmailbutton       , Locale["ma_Mail"]               , function() ArcAdmin:TogglePopup("mail", {}) end)
  --self:PrepareScript(ma_learnallbutton       , nil                             , function() ArcAdmin:LearnSpell("all") end)
  --self:PrepareScript(ma_learncraftsbutton    , nil                             , function() ArcAdmin:LearnSpell("all_crafts") end)
  --self:PrepareScript(ma_learngmbutton        , nil                             , function() ArcAdmin:LearnSpell("all_gm") end)
  --self:PrepareScript(ma_learnlangbutton      , nil                             , function() ArcAdmin:LearnSpell("all_lang") end)
  --self:PrepareScript(ma_learnclassbutton     , nil                             , function() ArcAdmin:LearnSpell("all_myclass") end)
  self:PrepareScript(ma_searchbutton         , nil                             , function() ArcAdmin:SearchStart("item", ma_searcheditbox:GetText()) end)
  self:PrepareScript(ma_resetsearchbutton    , nil                             , function() ArcAdmin:SearchReset() end)
  self:PrepareScript(ma_closebutton          , nil                             , function() ArcAdmin:CloseButton("bg") end)
  self:PrepareScript(ma_popupclosebutton     , nil                             , function() ArcAdmin:CloseButton("popup") end)
  self:PrepareScript(ma_popup2closebutton    , nil                             , function() ArcAdmin:CloseButton("popup2") end)
  self:PrepareScript(ma_inforefreshbutton    , nil                             , function() ArcAdmin:ChatMsg(".server info") end)
  self:PrepareScript(ma_frmtrslider          , Locale["tt_FrmTrSlider"]        , {{"OnMouseUp", function() ArcAdmin:ChangeTransparency("frames") end},{"OnValueChanged", function() ma_frmtrsliderText:SetText(string.format("%.2f", ma_frmtrslider:GetValue())) end}})  
  self:PrepareScript(ma_btntrslider          , Locale["tt_BtnTrSlider"]        , {{"OnMouseUp", function() ArcAdmin:ChangeTransparency("buttons") end},{"OnValueChanged", function() ma_btntrsliderText:SetText(string.format("%.2f", ma_btntrslider:GetValue())) end}})  
  self:PrepareScript(ma_mm_revivebutton      , nil                             , function() SendChatMessage(".revive", "GUILD", nil, nil) end)
end



function ArcAdmin:InitDropDowns()
  -- RELOAD TABLES
...
function ArcAdmin:InitSliders()
  ...
  ma_frmtrslider:SetValue(ArcAdmin.db.account.style.transparency.frames)
  ma_frmtrsliderText:SetText(string.format("%.2f", ArcAdmin.db.account.style.transparency.frames))
  ...
  ma_btntrslider:SetValue(ArcAdmin.db.account.style.transparency.buttons)
  ma_btntrsliderText:SetText(string.format("%.2f", ArcAdmin.db.account.style.transparency.buttons))
end

function ArcAdmin:InitScrollFrames()
  cont = ArcAdmin.db.char.selectedCont
  ...
  --local zoneupdate = function() Arc:TeleScrollUpdate() end
...
--[[  self:PrepareScript(ma_ticketeditbox, nil, {{"OnTextChanged", function() ScrollingEdit_OnTextChanged(self, ma_ticketeditbox) end},
  ...
  ma_maileditbox:SetScript("OnTextChanged", function() ArcAdmin:UpdateMailBytesLeft() end)
...
--[[  self:PrepareScript(ma_maileditbox, nil, {{"OnTextChanged", function() ScrollingEdit_OnTextChanged(self, ma_maileditbox); ArcAdmin:UpdateMailBytesLeft() end},
    {"OnCursorChanged", function() ScrollingEdit_OnCursorChanged(self, x, y, w, h) end},
    {"OnUpdate", function() ScrollingEdit_OnUpdate(self, 0, ma_maileditbox) end}})
]]
  ma_logframe:SetScript("OnUpdate", function() ArcAdminLogOnUpdate(self, 0, ma_logframe) end)
end

function ArcAdminLogOnUpdate(elapsedTime)
...
function ArcAdmin:NoResults(var)
  if var == "ticket" then
...
function PopupScrollUpdate()
  ...
  if ArcAdmin.db.char.requests.item or ArcAdmin.db.char.requests.favitem then --get items
    local count = 0
    if ArcAdmin.db.char.requests.item then
      table.foreachi(ArcAdmin.db.account.buffer.items, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favitem then
      table.foreachi(ArcAdmin.db.account.favorites.items, function() count = count + 1 end)
    end
    if count > 0 then
      ...
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local item
          if ArcAdmin.db.char.requests.item then
            item = ArcAdmin.db.account.buffer.items[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favitem then
            item = ArcAdmin.db.account.favorites.items[lineplusoffset]
          end
          ...
          getglobal("ma_PopupScrollBarEntryIcon"..line):SetScript("OnClick", function() ArcAdmin:AddItem(item["itId"], arg1) end)
          ...
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:AddItem(item["itId"], arg1) end)
          ...
          if ArcAdmin.db.char.requests.item then
            if item["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.items[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.items[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favitem then
            if item["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.items[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.items[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
      ...
      if ArcAdmin.db.char.requests.item then
        ArcAdmin:NoResults("search")
      elseif ArcAdmin.db.char.requests.favitem then
        ArcAdmin:NoResults("favorites")
      end
    end
    
  elseif ArcAdmin.db.char.requests.itemset or ArcAdmin.db.char.requests.favitemset then --get itemsets
    local count = 0
    if ArcAdmin.db.char.requests.itemset then
      table.foreachi(ArcAdmin.db.account.buffer.itemsets, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favitemset then
      table.foreachi(ArcAdmin.db.account.favorites.itemsets, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local itemset
          if ArcAdmin.db.char.requests.itemset then
            itemset = ArcAdmin.db.account.buffer.itemsets[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favitemset then
            itemset = ArcAdmin.db.account.favorites.itemsets[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Id: |cffffffff"..itemset["isId"].."|r Name: |cffffffff"..itemset["isName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:AddItemSet(itemset["isId"]) end)
          ...
          if ArcAdmin.db.char.requests.itemset then
            if itemset["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.itemsets[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.itemsets[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favitemset then
            if itemset["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.itemsets[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.itemsets[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.quest or ArcAdmin.db.char.requests.favquest then --get quests
    local count = 0
    if ArcAdmin.db.char.requests.quest then
      table.foreachi(ArcAdmin.db.account.buffer.quests, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favquest then
      table.foreachi(ArcAdmin.db.account.favorites.quests, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local quest
          if ArcAdmin.db.char.requests.quest then
            quest = ArcAdmin.db.account.buffer.quests[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favquest then
            quest = ArcAdmin.db.account.favorites.quests[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Id: |cffffffff"..quest["qsId"].."|r Name: |cffffffff"..quest["qsName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:Quest(quest["qsId"], arg1) end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):Enable()
          getglobal("ma_PopupScrollBarEntry"..line):Show()
          if ArcAdmin.db.char.requests.quest then
            if quest["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.quests[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.quests[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favquest then
            if quest["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.quests[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.quests[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.creature or ArcAdmin.db.char.requests.favcreature then --get creatures
    local count = 0
    if ArcAdmin.db.char.requests.creature then
      table.foreachi(ArcAdmin.db.account.buffer.creatures, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favcreature then
      table.foreachi(ArcAdmin.db.account.favorites.creatures, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local creature
          if ArcAdmin.db.char.requests.creature then
            creature = ArcAdmin.db.account.buffer.creatures[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favcreature then
            creature = ArcAdmin.db.account.favorites.creatures[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Id: |cffffffff"..creature["crId"].."|r Name: |cffffffff"..creature["crName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:Creature(creature["crId"], arg1) end) 
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):Enable()
          getglobal("ma_PopupScrollBarEntry"..line):Show()
          if ArcAdmin.db.char.requests.creature then
            if creature["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.creatures[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.creatures[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favcreature then
            if creature["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.creatures[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.creatures[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.spell or ArcAdmin.db.char.requests.favspell then --get spells
    local count = 0
    if ArcAdmin.db.char.requests.spell then
      table.foreachi(ArcAdmin.db.account.buffer.spells, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favspell then
      table.foreachi(ArcAdmin.db.account.favorites.spells, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local spell
          if ArcAdmin.db.char.requests.spell then
            spell = ArcAdmin.db.account.buffer.spells[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favspell then
            spell = ArcAdmin.db.account.favorites.spells[lineplusoffset]
          end
          local key = lineplusoffset
          --spell icon
          ...
          if ArcAdmin.db.char.requests.spell then
            if spell["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.spells[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.spells[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favspell then
            if spell["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.spells[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.spells[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.skill or ArcAdmin.db.char.requests.favskill then --get skills
    local count = 0
    if ArcAdmin.db.char.requests.skill then
      table.foreachi(ArcAdmin.db.account.buffer.skills, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favskill then
      table.foreachi(ArcAdmin.db.account.favorites.skills, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local skill
          if ArcAdmin.db.char.requests.skill then
            skill = ArcAdmin.db.account.buffer.skills[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favskill then
            skill = ArcAdmin.db.account.favorites.skills[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Id: |cffffffff"..skill["skId"].."|r Name: |cffffffff"..skill["skName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:SetSkill(skill["skId"], nil, nil) end)  
          getglobal("ma_PopupScrollBarEntry"..line):Enable()
          getglobal("ma_PopupScrollBarEntry"..line):Show()
          if ArcAdmin.db.char.requests.skill then
            if skill["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.skills[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.skills[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favskill then
            if skill["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.skills[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.skills[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.object or ArcAdmin.db.char.requests.favobject then --get objects
    local count = 0
    if ArcAdmin.db.char.requests.object then
      table.foreachi(ArcAdmin.db.account.buffer.objects, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favobject then
      table.foreachi(ArcAdmin.db.account.favorites.objects, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local object
          if ArcAdmin.db.char.requests.object then
            object = ArcAdmin.db.account.buffer.objects[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favobject then
            object = ArcAdmin.db.account.favorites.objects[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Id: |cffffffff"..object["objId"].."|r Name: |cffffffff"..object["objName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:AddObject(object["objId"], arg1) end)    
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):Enable()
          getglobal("ma_PopupScrollBarEntry"..line):Show()
          if ArcAdmin.db.char.requests.object then
            if object["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.objects[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.objects[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favobject then
            if object["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.objects[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.objects[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
  ...
  elseif ArcAdmin.db.char.requests.tele or ArcAdmin.db.char.requests.favtele then --get teles
    local count = 0
    if ArcAdmin.db.char.requests.tele then
      table.foreachi(ArcAdmin.db.account.buffer.teles, function() count = count + 1 end)
    elseif ArcAdmin.db.char.requests.favtele then
      table.foreachi(ArcAdmin.db.account.favorites.teles, function() count = count + 1 end)
    end
    if count > 0 then
      ma_lookupresulttext:SetText(Locale["searchResults"]..count)
      FauxScrollFrame_Update(ma_PopupScrollBar,count,7,30);
      for line = 1,7 do
        ...
        if lineplusoffset <= count then
          local tele
          if ArcAdmin.db.char.requests.tele then
            tele = ArcAdmin.db.account.buffer.teles[lineplusoffset]
          elseif ArcAdmin.db.char.requests.favtele then
            tele = ArcAdmin.db.account.favorites.teles[lineplusoffset]
          end
          local key = lineplusoffset
          getglobal("ma_PopupScrollBarEntry"..line):SetText("Name: |cffffffff"..tele["tName"].."|r")
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnClick", function() ArcAdmin:ChatMsg(".tele "..tele["tName"]) end)    
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnEnter", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):SetScript("OnLeave", function() --[[Do nothing]] end)
          getglobal("ma_PopupScrollBarEntry"..line):Enable()
          getglobal("ma_PopupScrollBarEntry"..line):Show()
          if ArcAdmin.db.char.requests.tele then
            if tele["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.teles[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.buffer.teles[key]["checked"] = true; PopupScrollUpdate() end)
            end
          elseif ArcAdmin.db.char.requests.favtele then
            if tele["checked"] then
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.teles[key]["checked"] = false; PopupScrollUpdate() end)
            else
              getglobal("ma_PopupScrollBarEntry"..line.."ChkBtn"):SetScript("OnClick", function() ArcAdmin.db.account.favorites.teles[key]["checked"] = true; PopupScrollUpdate() end)
            end
          end
...
function ArcAdmin:InlineScrollUpdate_temp()
    ma_ticketscrollframe:Hide()
    ArcAdmin:ChatMsg(".gmticket get")
    local ticketCount = 0
    table.foreachi(ArcAdmin.db.account.buffer.tickets, function() ticketCount = ticketCount + 1 end)
    if ticketCount > 0 then
        ArcAdmin:ChatMsg("TickCount"..ticketCount)
...
-- STYLE FUNCTIONS
function ArcAdmin:ToggleTransparency()
...
function ArcAdmin:ChangeTransparency(element)
  if element == "frames" then
    ArcAdmin.db.account.style.transparency.frames = string.format("%.2f", ma_frmtrslider:GetValue())
  elseif element == "buttons" then
    ArcAdmin.db.account.style.transparency.buttons = string.format("%.2f", ma_btntrslider:GetValue())
  end
end

function ArcAdmin:ToggleTooltips()
  if self.db.account.style.showtooltips then
...
function ArcAdmin:ToggleMinimenu()
...
function ArcAdmin:InitCheckButtons()
  if self.db.account.style.transparency.backgrounds < 1.0 then
  ...
  local dp = ArcAdmin.db.account.style.delayparam
...
function ArcAdmin:CloseButton(name)
  if name == "bg" then
    ArcAdmin:SearchReset()
    FrameLib:HandleGroup("bg", function(frame) frame:Hide() end)
