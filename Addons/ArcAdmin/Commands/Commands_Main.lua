...
function DisplayAccountLevel()
  MangAdmin:ChatMsg(".account info")
end
...
function ToggleHoverMode(value)
  MangAdmin:ChatMsg(".cheat hover "..value)
  local status
  if value == 1 then
    status = "on"
  else
...
function GridNavigate(x, y)
  ...
  if not x and not y then
    ...
    if pcall(function() return ma_gridnavieditbox:GetText() + 10 end) then
      ...
      MangAdmin:ChatMsg(".worldport "..newx.." "..newy)
...
function ToggleChat(value)
  MangAdmin:ChatMsg(".gm getid "..value)
...
function ToggleAccountlock(value)
  MangAdmin:ChatMsg(".account ban "..value)
...
function GoTrigger()
  local param = ma_parameter:GetText()
  MangAdmin:ChatMsg(".gotrid "..param)
...
function GoXYZ()
  local param = ma_parameter:GetText()
  MangAdmin:ChatMsg(".worldport "..param)
...
function AcctGMLvl()
  local param = ma_parameter:GetText()
  MangAdmin:ChatMsg(".account level "..param)
  MangAdmin:LogAction("Set account gmlevel: "..param)
end

function AcctPasswd()
  local param = ma_parameter:GetText()
  MangAdmin:ChatMsg(".account password "..param)
