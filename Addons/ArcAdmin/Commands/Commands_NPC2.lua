...
function NPCFreeze()
    ...
    MangAdmin:ChatMsg(".npc return")          
    MangAdmin:LogAction("Set NPC movement to STAY for player "..player..".")
end

function NPCFreezeDEL()
    local player = UnitName("target") or UnitName("player") 
    MangAdmin:ChatMsg(".npc stay")          
    MangAdmin:LogAction("Set NPC movement to STAY for player "..player..".")
end

function WayEndAdd()                            
    local player = UnitName("target") or UnitName("player")
    MangAdmin:ChatMsg(".waypoiny add")
...
function NPCAdd_Way()                            
    ...
    MangAdmin:ChatMsg(".waypoint add "..npc) 
    --MangAdmin:Way_Point_Add_Start_Write(1)
    MangAdmin:ChatMsg(".waypoint show "..npc)
    MangAdmin:LogAction("WayPoint Add for player "..player..".")
end

function WayModifyAdd()
    local player = UnitName("target") or UnitName("player")
    MangAdmin:ChatMsg(".waypoint add")
    MangAdmin:LogAction("WayPoint Add for player "..player..".")
end

function WayModifyDel()
    local player = UnitName("target") or UnitName("player")
    MangAdmin:ChatMsg(".waypoint del")
    MangAdmin:LogAction("WayPoint Del for player "..player..".")
end
...
function WayShowOn()
    local player = UnitName("target") or UnitName("player")
    MangAdmin:ChatMsg(".waypoint show on")
    MangAdmin:LogAction("WayPoint Show On for player "..player..".")
end

function WayShowOff()
    local player = UnitName("target") or UnitName("player")
    MangAdmin:ChatMsg(".waypoint hide")
